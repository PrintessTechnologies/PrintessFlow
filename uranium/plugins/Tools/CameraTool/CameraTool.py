...
class CameraTool(Tool):
    ...
    def _rotateCamera(self, x: float, y: float) -> None:
        ...
        # PRINTESS CHANGE: only suppress the tilt component when it moves the
        # camera TOWARD a pole (preventing the gimbal flip). Stock Cura also
        # suppressed tilts that move AWAY from the pole, which made rotating
        # out of an exact top view feel stuck: each small drag step stayed
        # inside the 0.1 rad guard zone and was discarded.
        if angle < 0.1 or angle > math.pi - 0.1:
            try:
                current_angle = math.acos(Vector.Unit_Y.dot(diff.normalized()))
            except ValueError:
                current_angle = angle
            moving_toward_pole = (angle < 0.1 and angle <= current_angle) or \
                                 (angle > math.pi - 0.1 and angle >= current_angle)
            if moving_toward_pole:
                n = diff.multiply(my)
